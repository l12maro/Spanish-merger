...
const consent_trial = {
	  ...
	  choices: ['Estoy de acuerdo', 'No estoy de acuerdo'],
	  ...
	  on_finish: function(data) {
        ...
        <div class="instruction-text">
          <h2>Gracias</h2>
          <p>Has elegido no participar. Gracias por tu tiempo.</p>
        </div>
      `);
	      }
	  }
  ...
  for (const q of questionItems) {
    ...
    if (q.scales) {
        ...
        if (q.id === 'q1' && condition === 'filler' && item && item.attention) {
          ...
          const attentionScale = { left: 'selecciona "uno"', right: 'selecciona "uno"', points: q.scales[mid] ? q.scales[mid].points : 6 };
